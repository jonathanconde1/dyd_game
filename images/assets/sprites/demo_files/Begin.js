...
function Being(x,y,key,e){
    // key is the string indicating which atlas to use
    console.log("in begin Phaser",Phaser);
    // console.log("key?",key);
    // Phaser.GameObjects.Sprite.call(e,x,y,key); // Call to constructor of parent
    this.speed = 0;
...
Being.prototype.attackPVP = function(target){ // Attack a target and display HP above it subsequently
    // hp is the integer of hit points to display
    // if(!this.target) return;
    // this.attack();
    // this.target.displayHP(hp);
    // if(!this.target) return;
    console.log("attackPVP=======");
    console.log(this);
    var direction = Game.adjacent(this,target);
...
Being.prototype.pathfindingCallback = function(finalOrientation,action,delta,sendToServer,path){
  console.log("begin ...");
    ...
    if(path === null && this.isPlayer) {
        ...
        if(action.action == 3 || action.action == 4|| action.action == 5){ // fight or chest
            finalOrientation = Game.computeFinalOrientation(path);
            // console.log("final orientation");
